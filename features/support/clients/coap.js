(function () {
  ...
  CoapClient = (function () {
    ...
    CoapClient.prototype.publish = function (topic, message, qos, callback) {
      let q = true
      if (qos === 'CON') q = false
      var req = coap.request({host: this.host, port: this.port, pathname: '/r/' + topic, method: 'post', confirmable: q})
      req.write(message)
      req.end()
      return callback()
    }
